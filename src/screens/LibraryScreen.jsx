...
import { BookOpen, ChevronRight, ChevronLeft, Sparkles, User, Loader2, CloudUpload, Database, Lock } from 'lucide-react';
...
export default function LibraryScreen({ navigateTo }) {
  ...
  const [progressMap, setProgressMap] = useState({}); // NUEVO: Estado del progreso
  ...
  const handleInjectToFirebase = async () => {
    ...
    try {
      for (const cap of libroDeMormon.chapters) {
        ...
        await setDoc(docRef, { id: cap.id, title: cap.chapterName, level: libroDeMormon.level, content: cap.content });
        count++;
      }
      alert(`¡Éxito! Se subieron ${count} capítulos a Firebase.`);
      window.location.reload(); 
    } catch (error) {
      console.error("Error inyectando datos:", error);
      alert("ERROR: Error en las reglas de base de datos.");
    } finally {
  ...
  useEffect(() => {
    // NUEVO: Cargar el progreso de lectura desde localStorage
    const loadProgress = () => {
      const newProgressMap = {};
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && key.startsWith('diliooo_progress_')) {
          const bookId = key.replace('diliooo_progress_', '');
          try {
            newProgressMap[bookId] = JSON.parse(localStorage.getItem(key));
          } catch (e) {
            newProgressMap[bookId] = { pIndex: parseInt(localStorage.getItem(key)) };
          }
        }
      }
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (userSnap.exists()) setIsPremium(userSnap.data().isPremium || false);
      } else {
        setIsPremium(false);
      }
    });
    ...
    const fetchBooks = async () => {
      try {
        ...
        const localMormonChapters = libroDeMormon.chapters.map(cap => ({
          id: cap.id, title: cap.chapterName, level: libroDeMormon.level, content: cap.content
        }));
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        <div className="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-[#2F6666] pb-6">
          ...
          <div className="flex items-center gap-3">
            {!isPremium && (
              <button onClick={() => navigateTo('pricing')} className="hidden md:flex items-center gap-1.5 px-4 py-2.5 bg-gradient-to-r from-[#FA8C7F] to-[#F19C83] text-white rounded-2xl text-xs font-black uppercase tracking-wider shadow-md hover:opacity-95 transition-opacity">
                ⭐ Ser Premium
              </button>
            )}

            <button onClick={() => navigateTo('admin')} className="p-3 bg-white dark:bg-[#2F6666] hover:bg-gray-50 dark:hover:bg-[#B55B49] text-gray-500 dark:text-[#EAE3D9] rounded-2xl border border-gray-200 dark:border-[#2F6666] transition-all shadow-sm flex items-center gap-2 text-sm font-bold">
              <Database className="w-4 h-4" /> <span className="hidden sm:inline">Admin</span>
            </button>

            <button onClick={handleInjectToFirebase} disabled={isInjecting} className={`p-3 bg-white dark:bg-[#2F6666] text-[#75A4A7] dark:text-[#F19C83] rounded-2xl border border-gray-200 dark:border-[#2F6666] transition-all shadow-sm flex items-center gap-2 text-sm font-bold ${isInjecting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50 dark:hover:bg-[#B55B49]'}`}>
            ...
            <button onClick={() => navigateTo('profile')} className="p-3 bg-white dark:bg-[#2F6666] hover:bg-gray-50 dark:hover:bg-[#B55B49] text-[#75A4A7] dark:text-[#EAE3D9] rounded-2xl border border-gray-200 dark:border-[#2F6666] transition-all shadow-sm flex items-center gap-2 text-sm font-bold">
              <User className="w-5 h-5" /> <span className="hidden sm:inline">Mi Perfil</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-14 mt-4">
          {sections.map((section) => {
            ...
            const isLevelLocked = section.key !== 'básico' && !isPremium;
            ...
            const groupedBooks = sectionBooksRaw.reduce((acc, book) => {
              ...
              if (mormonBooks.includes(bookName)) { chapterName = `${bookName} - ${chapterName}`; bookName = "El Libro de Mormón"; }

              if (!acc[bookName]) {
                acc[bookName] = { bookName, description: bookName === "El Libro de Mormón" ? "Explora todos los relatos y enseñanzas de este volumen de escritura sagrada." : `Explora todos los relatos y enseñanzas de "${bookName}".`, chapters: [] };
              }
              ...
              <div key={section.title} className="relative group/carousel w-full">
                <div className="max-w-6xl mx-auto px-4 md:px-8 mb-6 flex items-center justify-between">
                   <h2 className={`text-2xl font-black uppercase tracking-wider flex items-center gap-2 ${section.color}`}>
                     ...
                     {isLevelLocked && <span className="text-xs bg-amber-500/10 text-amber-500 px-2.5 py-1 rounded-full font-black tracking-widest uppercase">Premium</span>}
                   </h2>
                   
                   {groupedArray.length > 0 && (
                     <div className="hidden md:flex items-center gap-2">
                       <button onClick={() => scrollCarousel('left', carouselId)} className="p-2 rounded-full bg-white dark:bg-[#323435] border border-gray-200 dark:border-[#2F6666] text-gray-500 hover:bg-gray-50 dark:hover:bg-[#2F6666] transition-colors shadow-sm"><ChevronLeft className="w-5 h-5" /></button>
                       <button onClick={() => scrollCarousel('right', carouselId)} className="p-2 rounded-full bg-white dark:bg-[#323435] border border-gray-200 dark:border-[#2F6666] text-gray-500 hover:bg-gray-50 dark:hover:bg-[#2F6666] transition-colors shadow-sm"><ChevronRight className="w-5 h-5" /></button>
                     </div>
                   )}
                </div>

                {groupedArray.length === 0 ? (
                  <div className="max-w-6xl mx-auto px-4 md:px-8"><div className="bg-white dark:bg-[#2F6666]/30 border border-gray-100 dark:border-[#2F6666] rounded-3xl p-8 text-center shadow-sm"><p className="text-gray-400 dark:text-[#EAE3D9]/50 font-medium">Nuevos libros llegarán pronto.</p></div></div>
                ) : (
                  <div id={carouselId} className="flex gap-4 md:gap-6 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-6 px-4 md:px-8 items-start">
                    ...
                    {groupedArray.map((group) => {
                      ...
                      return (
                        <div key={group.bookName} className="shrink-0 snap-start w-[85vw] sm:w-[320px] md:w-[340px] bg-white dark:bg-transparent dark:bg-gradient-to-b dark:from-[#323435] dark:to-[#2a2b2c] rounded-3xl border border-gray-200 dark:border-[#2F6666] shadow-md transition-all duration-300 flex flex-col relative overflow-hidden">
                          <div onClick={() => toggleBook(group.bookName)} className="p-6 cursor-pointer group">
                            <div className="flex justify-between items-center mb-4">
                              <span className="px-3 py-1 bg-[#FA8C7F]/10 dark:bg-[#B55B49]/40 text-[#FA8C7F] dark:text-[#F19C83] rounded-full text-xs font-black uppercase tracking-wider shadow-sm">
                                {group.chapters.length} {group.chapters.length === 1 ? 'Lectura' : 'Lecturas'}
                              </span>
                              <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-300 shadow-sm border border-gray-100 dark:border-[#2F6666] ${isExpanded ? 'bg-[#FA8C7F] dark:bg-[#F19C83] rotate-90' : 'bg-gray-50 dark:bg-[#323435]'}`}>
                                <ChevronRight className={`w-4 h-4 transition-colors ${isExpanded ? 'text-white dark:text-[#323435]' : 'text-gray-400 dark:text-[#EAE3D9]'}`} />
                              </div>
                            </div>
                            <h3 className="text-xl font-black text-gray-900 dark:text-[#EAE3D9] group-hover:text-[#FA8C7F] dark:group-hover:text-[#F19C83] transition-colors mb-2 tracking-tight leading-tight">{group.bookName}</h3>
                            <p className="text-gray-500 dark:text-[#EAE3D9]/70 text-sm leading-relaxed font-medium line-clamp-2">{group.description}</p>
                          </div>
                          ...
                          {isExpanded && (
                            <div className="bg-gray-50 dark:bg-[#2F6666]/30 border-t border-gray-100 dark:border-[#2F6666] animate-fade-in-up max-h-[300px] overflow-y-auto no-scrollbar">
                              {group.chapters.map((chapter, idx) => {
                                // NUEVO: Evaluamos si este capítulo en particular tiene progreso de lectura
                                const progressInfo = progressMap[chapter.id];
                                ...
                                return (
                                  <div 
                                    ...
                                    onClick={(e) => { 
                                      e.stopPropagation(); 
                                      if (isLevelLocked) { navigateTo('pricing'); return; }
                                      navigateTo('reader', chapter); 
                                    }}
                                    className="px-6 py-4 flex items-center justify-between border-b border-gray-200/50 dark:border-[#2F6666]/50 hover:bg-white dark:hover:bg-[#2F6666]/80 transition-colors cursor-pointer last:border-0 group/chapter relative overflow-hidden"
                                  >
                                    <div className="flex items-center gap-3">
                                      <span className={`font-black text-sm opacity-60 transition-colors ${progressInfo ? 'text-[#FA8C7F] dark:text-[#F19C83]' : 'text-[#75A4A7] dark:text-[#EAE3D9]'}`}>
                                        {(idx + 1).toString().padStart(2, '0')}
                                      </span>
                                      <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                          <span className={`font-bold text-sm transition-colors line-clamp-1 ${progressInfo ? 'text-[#FA8C7F] dark:text-[#F19C83]' : 'text-gray-700 dark:text-[#EAE3D9] group-hover/chapter:text-[#FA8C7F] dark:group-hover/chapter:text-white'}`}>
                                            {chapter.chapterName}
                                          </span>
                                          {progressInfo && <span className="text-[9px] bg-[#FA8C7F]/10 text-[#FA8C7F] dark:bg-[#F19C83]/20 dark:text-[#F19C83] px-2 py-0.5 rounded-full uppercase tracking-wider font-black shrink-0">Leyendo</span>}
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    {isLevelLocked ? (
                                      ...
                                      <BookOpen className={`w-4 h-4 shrink-0 transition-colors ${progressInfo ? 'text-[#FA8C7F] dark:text-[#F19C83]' : 'text-gray-300 dark:text-[#2F6666] group-hover/chapter:text-[#FA8C7F] dark:group-hover/chapter:text-[#F19C83]'}`} />
                                    )}
                                  </div>
                                );
                              })}
